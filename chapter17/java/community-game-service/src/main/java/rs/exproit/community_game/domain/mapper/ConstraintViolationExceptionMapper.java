package rs.exproit.community_game.domain.mapper;
...
public class ConstraintViolationExceptionMapper
        implements JsonApiExceptionMapper<ConstraintViolationException> {
    @Override
    public ErrorResponse toErrorResponse(ConstraintViolationException cve) {
        ...
        for (ConstraintViolation<?> violation : cve.getConstraintViolations()) {
            ErrorData error = ErrorData.builder()
                    ...
                    .setSourcePointer(createSourcePointer(violation)).build();

            errors.add(error);
        }

        return ErrorResponse.builder().setStatus(CONFLICT_409)
    ...
    private String findAttributeName(ConstraintViolation<?> violation) {
        for (Iterator<Node> iterator = violation.getPropertyPath()
